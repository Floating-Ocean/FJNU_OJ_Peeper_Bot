...
from botpy import BotAPI, logging
from botpy.ext.cog_yaml import read
from botpy.message import Message, GroupMessage

from utils.tools import run_async
...
_key_words = {
    "傻逼": ["谢谢夸奖", "反弹", "可能还真被你说对了", "嗯", "好的", "哼，你才是", "哈哈", "你可能说对了，你可能没说对"],
    ...
    async def reply(self, content: str, img_path: str = None, img_url: str = None, modal_words: bool = True):
        ...
        if self.guild:  # 频道消息
            run_async(self.main_event_loop,
                      self.api.post_message(channel_id=self.guild_message.channel_id, msg_id=self.guild_message.id,
            ...
            if img_path is not None or img_url is not None:
                ...
                while media is None and retry_times < 3:
                    if img_path is not None:
                        ...
                        media = run_async(self.main_event_loop,
                                          self.api.post_group_file(group_openid=self.group_message.group_openid,
                                                                   file_type=1,
                ...
                if media is None:
                    run_async(self.main_event_loop,
                              self.api.post_group_message(group_openid=self.group_message.group_openid,
