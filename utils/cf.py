...
async def get_prob_filter_tag(me, message, tag_needed, limit=None, newer=False):
    if tag_needed == "all":
        url = f"https://codeforces.com/api/problemset.problems"
    else:
        ...
        if tag_needed not in all_tags:
            ...
            await reply(me, message, f"最佳匹配 Tag: {tag_needed}")
    ...
    filtered_data = [prob for prob in json_data['result']['problems'] if
                     'rating' in prob and min_point <= prob['rating'] <= max_point] if limit is not None else json_data['result']['problems']
...
def format_verdict(verdict, passed_count):
    if verdict == "OK":
        return "ACCEPTED"
    else:
...
async def get_user_last_submit(handle):
...
async def send_user_info(me, message, handle):
...
Recent Submit:{last_submit}"""
    await reply(me, message, content)


async def send_prob_tags(me, message):
    prob_tags = await get_prob_tags()
...
async def send_prob_filter_tag(me, message, tag, limit=None, newer=False):
    chosen_prob = await get_prob_filter_tag(me, message, tag, limit, newer)
...
async def reply_cf_request(me, message):
    content = re.sub(r'<@!\d+>', '', message.content).strip().split()
    if content[1] == "info":
        if len(content) != 3:
            await reply(me, message, "请输入正确的指令格式，如 /cf info jiangly")
            return
        await send_user_info(me, message, content[2])
    elif content[1] == "prob":
        if content[2] == "pick":
            await send_prob_filter_tag(me, message, content[3], content[4] if len(content) >= 5 and content[4] != "new" else None, content[4] == "new" if len(content) == 5 else (content[5] == "new" if len(content) == 6 else False))
        elif content[2] == "tags":
            await send_prob_tags(me, message)
        else:
            await reply(me, message, "请输入正确的指令格式，如:\n\n"
                                           "/cf prob pick dp 1700-1900\n"
                                           "/cf prob pick dfs-and-similar\n"
                                           "/cf prob pick all 1800\n"
                                           "/cf prob tags")
    else:
        await reply(me, message, "目前仅支持 info 和 prob 指令\n\ninfo:\n/cf info [用户名]\n\n"
