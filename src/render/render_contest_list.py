...
import pixie
from easy_pixie import StyledString, calculate_height, draw_text, calculate_string_width, draw_rect, Loc, draw_img
...
class ContestListRenderer(Renderer):
    ...
    class _ContestItem:
        def __init__(self, contest: Contest, idx: int):
            ...
            self._begin_x = 108 + calculate_string_width(self._00_idx_text) + 48
            max_width = 1024 + 32 - self._begin_x - 48 - 108

            self._idx_text = StyledString(f"{self._idx:02d}",
                                          'H', 78,
                                          font_color=(0, 0, 0, 30 / 255), padding_bottom=12)
        ...
        def render_item(self, img: pixie.Image, y: int) -> int:
            current_x, current_y = self._begin_x - calculate_string_width(self._idx_text) - 48, y
    ...
    def __init__(self, upcoming_contests: list[Contest], running_contests: list[Contest], finished_contests: list[Contest]):
    ...
    def render(self) -> pixie.Image:
        title_text = StyledString("近日比赛", 'H', 96, padding_bottom=8)
        copyright_text = StyledString("Generated by OBot. 功能性测试，不代表最终结果.", 'H', 28, padding_bottom=96)
        none_title_text = StyledString("最近没有比赛，放松一下吧", 'H', 52)
        upcoming_title_text = StyledString(">> 即将进行的比赛 >>", 'H', 52, padding_bottom=72)
        running_title_text = StyledString(">> 正在进行的比赛 >>", 'H', 52, padding_bottom=72)
        finished_title_text = StyledString(">> 已结束的比赛 >>", 'H', 52, padding_bottom=72)

        width, height = 1024, calculate_height([title_text, copyright_text]) + 256
        if len(self._upcoming_contests) == 0 and len(self._running_contests) == 0 and len(self._finished_contests) == 0:
            ...
            height -= 128
            for _contests, _type_title_text in [(self._upcoming_contests, upcoming_title_text),
                                                (self._running_contests, running_title_text),
                                                (self._finished_contests, finished_title_text)]:
                if len(_contests) > 0:
                    height += calculate_height([_type_title_text])
                    height += 128 + sum([contest.calculate_height() for contest in _contests])
                    height += 108 * (len(_contests) - 1)  # 各比赛间的 padding

        img = pixie.Image(width + 64, height + 64)
        img.fill(pixie.Color(0, 0, 0, 1))  # 填充黑色背景

        paint_bg = pixie.Paint(pixie.SOLID_PAINT)
        paint_bg.color = pixie.Color(0.99, 0.99, 0.99, 1)
        draw_rect(img, paint_bg, Loc(32, 32, 1024, height), 96)

        current_x, current_y = 108, 128
        current_y = draw_text(img, title_text, current_x, current_y)
        current_y = draw_text(img, copyright_text, current_x, current_y)
        ...
        if len(self._upcoming_contests) == 0 and len(self._running_contests) == 0 and len(self._finished_contests) == 0:
            ...
            for _contests, _type_title_text in [(self._upcoming_contests, upcoming_title_text),
                ...
                if len(_contests) > 0:
                    current_y += 128
                    current_y = draw_text(img, _type_title_text, current_x, current_y)
                    current_y -= 108
